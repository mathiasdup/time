...
function smoothCloseOppHandGap(hiddenCard) {
    ...
    // Capture sibling positions before collapse
    var siblings = [];
    var oldLefts = [];
    var children = panel.children;
    for (var i = 0; i < children.length; i++) {
        if (children[i] !== hiddenCard && children[i].style.width !== '0px') {
            siblings.push(children[i]);
            oldLefts.push(children[i].getBoundingClientRect().left);
        }
    }

    // Instant collapse (no CSS transition) for clean layout
    ...
    // Force reflow to get final positions
    panel.getBoundingClientRect();

    // FLIP: snap siblings to old positions, then animate to new
    var toAnimate = [];
    for (var j = 0; j < siblings.length; j++) {
        var newLeft = siblings[j].getBoundingClientRect().left;
        var dx = oldLefts[j] - newLeft;
        if (Math.abs(dx) > 1) {
            siblings[j].style.transition = 'none';
            siblings[j].style.transform = 'translateX(' + dx + 'px)';
            toAnimate.push(siblings[j]);
        }
    }
    ...
    if (toAnimate.length > 0) {
        ...
        requestAnimationFrame(function() {
            for (var k = 0; k < toAnimate.length; k++) {
                toAnimate[k].style.transition = 'transform 0.35s cubic-bezier(0.4, 0, 0.2, 1)';
                toAnimate[k].style.transform = '';
            }
            setTimeout(function() {
                for (var m = 0; m < toAnimate.length; m++) {
                    toAnimate[m].style.transition = '';
                    toAnimate[m].style.transform = '';
                }
            }, 400);
        });
    }
...
async function animateSpellReveal(card, casterPlayerNum, startRect = null) {
    ...
    // Opp spells: z-index 10000 directly (lift already cleared stacking)
    if (oppFlip) {
        var _zTarget = perspContainer || wrapper;
        _zTarget.style.zIndex = '10000';
    }
            ...
            if (oppAnimIndex !== null || !!preferredSpellUid) {
                ...
                if (resolved.el && resolved.rect) {
                    ...
                    if (resolved.mode === 'uid-match' || resolvedByIndex) {
                        ...
                        resolved.el.style.transform = 'translateY(-230px)';
                        await new Promise(function(r) { setTimeout(r, 190); });
                        // Capture lifted rect as source position
                        if (resolved.el.isConnected) {
                            savedRect = resolved.el.getBoundingClientRect();
                        }
                        delete resolved.el.dataset.lifting;
...
function flyFromOppHand(targetRect, duration = 300, spell = null, savedSourceRect = null, options = {}) {
    return new Promise(resolve => {
        ...
        function _doFlyAfterLift() {
            ...
            if (_sourceEl && _sourceEl.isConnected) {
                delete _sourceEl.dataset.lifting;
            ...
            var fw = targetRect.width, fh = targetRect.height;
            var flyCard = document.createElement('div');
            flyCard.style.cssText = 'position: fixed; z-index: 10001; pointer-events: none; overflow: hidden;'
            ...
            if (spell && spell.revealedToOpponent && typeof makeCard === 'function') {
                var cardFace = makeCard(spell, false);
                cardFace.style.width = '100%';
                cardFace.style.height = '100%';
                cardFace.style.margin = '0';
            ...
            // Bezier trajectory from lifted position
            var scx = handRect.left + handRect.width / 2;
            var scy = handRect.top + handRect.height / 2;
            var ecx = targetRect.left + fw / 2;
            var ecy = targetRect.top + fh / 2;
            var ccx = (scx + ecx) / 2;
            var ccy = Math.max(scy, ecy) + 50;

            var t0 = performance.now();
            function animate() {
                var p = Math.min((performance.now() - t0) / duration, 1);
                var t = easeInOutCubic(p);

                var cx = (1-t)*(1-t)*scx + 2*(1-t)*t*ccx + t*t*ecx;
                var cy = (1-t)*(1-t)*scy + 2*(1-t)*t*ccy + t*t*ecy;
        ...
        if (_sourceEl && _sourceEl.isConnected) {
            _sourceEl.dataset.lifting = 'true';
            _sourceEl.style.transition = 'transform 0.16s ease-out';
            _sourceEl.style.transform = 'translateY(-230px)';
            setTimeout(_doFlyAfterLift, 190);
        } else {
            // No element to lift - offset handRect manually for visual lift
            handRect = { left: handRect.left, top: handRect.top - 230, width: handRect.width, height: handRect.height };
            _doFlyAfterLift();
        }
    });
}

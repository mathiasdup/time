...
async function animateBurn(data) {
    ...
    const burnZoom = 1.2;
    const zoomOffX = cardWidth * (burnZoom - 1) / 2;
    const zoomOffY = cardHeight * (burnZoom - 1) / 2;
    ...
    // Wrapper avec perspective + zoom pour netteté (re-rasterise au lieu de bitmap-scale)
    const wrapper = document.createElement('div');
    wrapper.style.cssText = `
        position: fixed; z-index: 10000; pointer-events: none;
        left: ${startX - zoomOffX}px; top: ${startY - zoomOffY}px;
        width: ${cardWidth}px; height: ${cardHeight}px;
        zoom: ${burnZoom};
        transform-origin: center center;
        transform: scale(${1 / burnZoom}); opacity: 0;
        perspective: 800px;
    `;
    ...
    if (graveTopEl2) {
        ...
        for (let pass = 0; pass < 3; pass++) {
            wrapper.style.left = (graveX - zoomOffX) + 'px';
            wrapper.style.top = (graveY - zoomOffY) + 'px';
    ...
    await new Promise(resolve => {
        ...
        function animate() {
            ...
            if (progress <= t1) {
                ...
                x = startX;
                y = startY - ep * 30;
                scaleX = scaleY = (1 + ep * 0.05) / burnZoom;
                opacity = 0.3 + ep * 0.7;
                flipDeg = 0;
                redTint = 0;
                tiltDeg = 0;
                ...
                scaleX = scaleY = (1.05 + (1.2 - 1.05) * ep) / burnZoom;
                opacity = 1;
            ...
            wrapper.style.left = (x - zoomOffX) + 'px';
            wrapper.style.top = (y - zoomOffY) + 'px';
...
async function animateDeathToGraveyard(data) {
    ...
    const cardWidth = 140;
    const cardHeight = 189;
    const deathZoom = slotRect.width / cardWidth;
    const zoomOffX = cardWidth * (deathZoom - 1) / 2;
    const zoomOffY = cardHeight * (deathZoom - 1) / 2;
    ...
    // 4. Créer le wrapper avec la carte + zoom pour netteté
    const wrapper = document.createElement('div');
    wrapper.style.cssText = `
        position: fixed; z-index: 10000; pointer-events: none;
        left: ${startX - zoomOffX}px; top: ${startY - zoomOffY}px;
...
async function animateSpellReveal(card, casterPlayerNum, startRect = null) {
    ...
    const spellZoom = showcaseScale;
    const zoomOffX = cardWidth * (spellZoom - 1) / 2;
    const zoomOffY = cardHeight * (spellZoom - 1) / 2;
    const baseScale = 1 / spellZoom;
